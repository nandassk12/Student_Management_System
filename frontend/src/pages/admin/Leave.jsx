...
export default function AdminLeave() {
  ...
  // ── Query: Fetch teacher leave requests ─────────────────────────
  const { data: leaves = [], isLoading, error } = useQuery({
    queryKey: ['adminTeacherLeaveRequests'],
    queryFn: async () => {
      const { data } = await axiosInstance.get('/teacher/leave')
      return data
    }
  })

  // ── Mutation: Review teacher leave request ───────────────────────────────────
  const reviewLeaveMutation = useMutation({
    mutationFn: async ({ id, status }) => {
      const { data } = await axiosInstance.post(`/teacher/leave/${id}/review`, { status })
      return data
    },
    onSuccess: (data) => {
      toast.success(`Leave request was successfully ${data.status}!`)
      queryClient.invalidateQueries({ queryKey: ['adminTeacherLeaveRequests'] })
    },
    onError: (err) => {
  ...
  return (
    <div className="space-y-6 page-enter">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-extrabold text-text-primary tracking-tight">Teacher Leave Requests</h1>
        <p className="text-sm text-text-muted mt-1">Review, approve, or reject absence notifications submitted by teachers.</p>
      </div>
      ...
      <div className="card bg-white rounded-t-none border-t-0 overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-12 text-center text-text-muted flex flex-col items-center justify-center space-y-2">
            ...
            <p className="text-xs max-w-xs mt-1">There are no {activeTab} teacher leave requests awaiting your attention.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-card-border bg-slate-50/20">
                  <th className="p-4 text-xs font-bold text-text-secondary uppercase tracking-wider">Teacher</th>
                      ...
                      <td className="p-4 align-top w-64">
                        <div className="flex items-center gap-3">
                          <div className="w-9 h-9 rounded-full bg-navy-50 text-primary border border-navy-100 flex items-center justify-center text-xs font-extrabold flex-shrink-0">
                            {(teacher.full_name || teacher.username || '??').substring(0, 2).toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <h4 className="text-sm font-bold text-text-primary truncate">
                              {teacher.full_name || teacher.username}
                            </h4>
                            <p className="text-xs text-text-muted mt-0.5 truncate">{teacher.email}</p>
                          </div>
                        </div>
                      </td>
