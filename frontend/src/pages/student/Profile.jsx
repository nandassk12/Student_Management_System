...
export default function StudentProfile() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      setAcademicForm({
        ...
        hostel_status: profile.hostel_status || ''
      })
  ...
  const saveIdentity = async (e) => {
    e.preventDefault()
    const toastId = toast.loading('Saving identity details...')
    try {
      const { data } = await updateStudentProfile(identityForm)
      queryClient.setQueryData(['studentProfileSelf'], data)
  ...
  const saveAcademic = async (e) => {
    e.preventDefault()
    const toastId = toast.loading('Saving academic details...')
    try {
      const payload = {
        year_of_study: Number(academicForm.year_of_study) || null,
        batch_year: Number(academicForm.batch_year) || null,
        hostel_status: academicForm.hostel_status
      }
      const { data } = await updateStudentProfile(payload)
      queryClient.setQueryData(['studentProfileSelf'], data)
      toast.success('Academic data updated!', { id: toastId })
    } catch (err) {
      toast.error(err.response?.data?.detail || 'Failed to save academic details.', { id: toastId })
    }
  }
  ...
  const handlePhotoSelect = async (e) => {
    ...
    try {
      const { data } = await uploadStudentPhoto(formData)
      queryClient.setQueryData(['studentProfileSelf'], data)
  ...
  const handleSignatureSelect = async (e) => {
    ...
    try {
      const { data } = await uploadStudentSignature(formData)
      queryClient.setQueryData(['studentProfileSelf'], data)
  ...
  const uploadDoc = async (docType, file) => {
    ...
    // Check if docType is signature (which uses a distinct API endpoint)
    if (docType === 'signature') {
      const formData = new FormData()
      formData.append('file', file)
      const toastId = toast.loading('Uploading signature to vault...')
      try {
        const { data } = await uploadStudentSignature(formData)
        queryClient.setQueryData(['studentProfileSelf'], data)
        toast.success('Signature uploaded successfully!', { id: toastId })
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['studentDocumentsSelf'] })
  ...
  const deleteDoc = async (docId, docType) => {
    // If docType is signature, delete means setting signature field to null via updateProfile
    if (docType === 'signature') {
      const toastId = toast.loading('Removing signature...')
      try {
        const { data } = await updateStudentProfile({ signature: null })
        queryClient.setQueryData(['studentProfileSelf'], data)
        toast.success('Signature removed.', { id: toastId })
      } catch (err) {
        toast.error(err.response?.data?.detail || 'Failed to remove signature.', { id: toastId })
      }
      return
    }
  ...
  const handlePreview = async (docId, fileName, isSignature) => {
    if (isSignature) {
      if (profile?.signature) {
        window.open(getFileUrl(profile.signature), '_blank')
      } else {
        toast.error('No signature file to preview.')
      }
      return
    }
    ...
    try {
      const response = await downloadStudentDocumentSelf(docId)
  ...
  const handleDownload = async (docId, fileName, isSignature) => {
    if (isSignature) {
      if (profile?.signature) {
        const link = document.createElement('a')
        link.href = getFileUrl(profile.signature)
        link.setAttribute('download', profile.signature.split(/[/\\]/).pop())
        document.body.appendChild(link)
        link.click()
        link.remove()
      } else {
        toast.error('No signature file to download.')
      }
      return
    }
  ...
  const docVaultTypes = [
    { key: 'signature', label: 'Student Signature', isSignature: true },
    { key: 'aadhaar', label: 'Aadhaar Card' },
    { key: 'pan', label: 'PAN Card' },
    { key: '10th_cert', label: '10th Marksheet/Certificate' },
    { key: '12th_cert', label: '12th Marksheet/Certificate' },
    { key: 'resume', label: 'Student Resume/CV' }
  ]
  ...
  const displayName = `${identityForm.first_name} ${identityForm.last_name}`.trim() || profile?.user?.username || 'Student Profile'
  ...
  return (
    <div className="space-y-8 page-enter max-w-4xl pb-16">
      ...
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">Review academic credentials, sync contact coordinates, upload verification vaults, or configure passwords.</p>
      </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-5 w-full md:w-auto">
          ...
          <div className="text-center sm:text-left space-y-1.5">
            <div className="flex flex-col sm:flex-row sm:items-center gap-2">
              ...
              <span className="inline-block px-2.5 py-0.5 text-[9px] font-black rounded-full uppercase tracking-wider bg-green-50 border border-green-200 text-status-green self-center">
                {profile?.user?.role?.name || 'Student'}
              </span>
            </div>
            ...
            {profile?.roll_number && (
              <p className="text-xs font-bold text-text-secondary">
                Roll Number: {profile.roll_number} {profile.class_ ? `• ${profile.class_.name}` : ''}
              </p>
            )}
          </div>
        </div>
      ...
      <div className="card bg-white p-6 space-y-6 border-[#e2e8f0]">
        <div className="border-b border-slate-100 pb-4">
          <h3 className="text-base font-bold text-text-primary">Identity Info</h3>
          <p className="text-xs text-text-muted mt-0.5">Edit credentials and basic identification values.</p>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) => setIdentityForm({ ...identityForm, first_name: e.target.value })}
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) => setIdentityForm({ ...identityForm, last_name: e.target.value })}
                className="input font-medium"
                required
              />
            </div>
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Blood Group</label>
              <select
                value={identityForm.blood_group}
                onChange={(e) => setIdentityForm({ ...identityForm, blood_group: e.target.value })}
                className="input bg-white font-medium"
              >
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Nationality</label>
              <input
                type="text"
                value={identityForm.nationality}
                placeholder="e.g. Indian"
                onChange={(e) => setIdentityForm({ ...identityForm, nationality: e.target.value })}
                className="input font-medium"
              />
            </div>

            <div className="space-y-1 md:col-span-2">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">State / Origin</label>
              <input
                type="text"
                value={identityForm.state}
                placeholder="e.g. Karnataka"
                onChange={(e) => setIdentityForm({ ...identityForm, state: e.target.value })}
                className="input font-medium"
              />
            </div>
          </div>
      ...
      <div className="card bg-white p-6 space-y-6 border-[#e2e8f0]">
        <div className="border-b border-slate-100 pb-4">
          <h3 className="text-base font-bold text-text-primary">Academic Metadata</h3>
          <p className="text-xs text-text-muted mt-0.5">Read-only system records and configurable enrollment status parameters.</p>
        </div>
        ...
        <form onSubmit={saveAcademic} className="space-y-5">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-slate-50/50 p-4 rounded-xl border border-slate-100">
            ...
            <div className="space-y-1">
              <span className="text-[10px] font-black text-text-muted uppercase tracking-wider">Class / Cohort</span>
              <p className="text-sm font-semibold text-text-primary">{profile?.class_?.name || '—'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[10px] font-black text-text-muted uppercase tracking-wider">Roll Number</span>
              <p className="text-sm font-semibold text-text-primary font-mono">{profile?.roll_number || '—'}</p>
            </div>
            <div className="space-y-1">
              <span className="text-[10px] font-black text-text-muted uppercase tracking-wider">Admission Date</span>
              <p className="text-sm font-semibold text-text-primary">
                {profile?.admission_date ? new Date(profile.admission_date).toLocaleDateString(undefined, { dateStyle: 'long' }) : '—'}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Batch Year</label>
              <input
                type="number"
                value={academicForm.batch_year}
                placeholder="e.g. 2026"
                onChange={(e) => setAcademicForm({ ...academicForm, batch_year: e.target.value })}
                className="input font-medium"
              />
            </div>

            <div className="space-y-1">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Hostel Status</label>
              <select
                value={academicForm.hostel_status}
                onChange={(e) => setAcademicForm({ ...academicForm, hostel_status: e.target.value })}
                className="input bg-white font-medium"
              >
      ...
      {/* ── SECTION 3: CONTACT & PARENT DETAILS ── */}
      <div className="card bg-white p-6 space-y-6 border-[#e2e8f0]">
        <div className="border-b border-slate-100 pb-4">
          <h3 className="text-base font-bold text-text-primary">Contact & Family Coordinates</h3>
          <p className="text-xs text-text-muted mt-0.5">Address sync parameters, emergency points, and parent logs.</p>
        </div>
        ...
        <form onSubmit={saveContact} className="space-y-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1">
              <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Mobile Phone</label>
              <input
                type="text"
          ...
          <div className="space-y-1">
            <label className="text-xs font-bold text-text-secondary uppercase tracking-wider">Home Address (Short Summary)</label>
            <input
              type="text"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <textarea
                ...
                placeholder="Current hostel or local residence..."
              />
            </div>
          ...
          <div className="space-y-3">
            <span className="text-xs font-bold text-text-secondary uppercase tracking-wider block">Parent / Guardian Information</span>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-text-secondary uppercase">Parent Full Name</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                ...
                <input
                  type="text"
                  value={contactForm.parent_relationship}
                  onChange={(e) => setContactForm({ ...contactForm, parent_relationship: e.target.value })}
                  className="input py-1.5 text-xs"
                />
              </div>
              <div className="space-y-1">
          ...
          <div className="space-y-3">
            <span className="text-xs font-bold text-text-secondary uppercase tracking-wider block">Emergency Contact</span>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 p-4 rounded-xl border border-slate-100">
              <div className="space-y-1">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-text-secondary uppercase">Emergency Phone</label>
                <input
                  type="text"
      ...
      <div className="card bg-white p-6 space-y-6 border-[#e2e8f0]">
        <div className="border-b border-slate-100 pb-4">
          ...
          <p className="text-xs text-text-muted mt-0.5">Submit verification certificates and official files for compliance.</p>
        </div>
        ...
        <div className="divide-y divide-slate-100 border border-slate-200 rounded-xl overflow-hidden bg-white">
          {docVaultTypes.map((type) => {
            ...
            if (type.isSignature) {
              uploadedDoc = profile?.signature ? {
                id: 'signature',
                file_name: profile.signature.split(/[/\\]/).pop(),
                uploaded_at: profile.last_edited_at
              } : null
            } else {
              uploadedDoc = documents.find(d => d.doc_type.toLowerCase() === type.key)
            }
                    ...
                    {uploadedDoc ? (
                      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-3 border border-green-100 bg-green-50/5 rounded-lg">
                        ...
                        <div className="flex flex-wrap gap-2 w-full sm:w-auto justify-end">
                          <button
                            type="button"
                            onClick={() => handlePreview(uploadedDoc.id, uploadedDoc.file_name, type.isSignature)}
                          ...
                          <button
                            type="button"
                            onClick={() => handleDownload(uploadedDoc.id, uploadedDoc.file_name, type.isSignature)}
                          ...
                          <button
                            type="button"
                            onClick={() => deleteDoc(uploadedDoc.id, type.key)}
      ...
      <div className="card bg-white p-6 space-y-6 border-[#e2e8f0]">
        <div className="border-b border-slate-100 pb-4">
          ...
          <p className="text-xs text-text-muted mt-0.5">Configure authentication parameters and portal access tokens.</p>
        </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute inset-y-0 right-0 pr-3 flex items-center text-text-muted hover:text-text-primary transition-colors focus-visible:outline-none"
              >
                {showCurrentPassword ? (
                    ...
                    {[1, 2, 3, 4].map((step) => (
                      <div
                        ...
                        className={`h-full rounded-full transition-colors ${
                          pwStrength >= step ? strengthColors[pwStrength] : 'bg-slate-200'
                        }`}
                      />
                    ))}
                  </div>
